...
def split_fortran_units_simple(text: str) -> List[Dict[str, object]]:
    ...
    def _parse_proc_header(code: str):
        m = proc_hdr_re.match(code)
        if not m:
            return None
        kind = m.group(1).lower()
        name = m.group(2)
        arg_text = (m.group(3) or "").strip()
        args = [a.strip() for a in arg_text[1:-1].split(",") if a.strip()] if arg_text else []
        result = m.group(4).strip() if (kind == 'function' and m.group(4)) else None
        return kind, name, args, result

    def _parse_unit(i: int):
        hdr_lineno, code = stmts[i]
        low = code.lower()
        prog = prog_hdr_re.match(low)
        ph = _parse_proc_header(low)
        if ph:
            kind, name, args, result = ph
            end_re = re.compile(rf"^end\s+{kind}\b", re.IGNORECASE)
        elif prog:
            kind, name, args, result = 'program', prog.group(1), [], None
            end_re = re.compile(r"^end\s+program\b", re.IGNORECASE)
        else:
            return [], i + 1
    ...
    while i < len(stmts):
        ...
        if low == 'contains':
            i += 1
            continue
        units_here, next_i = _parse_unit(i)
        if units_here:
            out.extend(units_here)
            i = next_i
            continue
        i += 1
    if not out:
        ...
        for lineno, stmt in stmts:
            ...
            if low in {'end', 'end program'}:
                continue
            if stmt:
                body.append(stmt)
                body_line_nos.append(lineno)
        if body:
            out.append({
                'kind': 'program',
                'name': 'main',
    ...
    def _local_derived_info(body: List[str]) -> Tuple[Set[str], List[Tuple[int, int]]]:
        ...
        for idx, stmt in enumerate(body):
            ...
            if not in_type:
                ...
                if m_type:
                    names.add(m_type.group(1).lower())
                    in_type = True
                    start_idx = idx
                continue
    ...
    for lineno, stmt in iter_fortran_statements(lines):
        ...
        if re.match(
            ...
            if not unit_stack:
                in_implicit_main = True
            continue
        if re.match(r"^type\s+[a-z_]\w*\s*$", low):
            if not unit_stack:
                in_implicit_main = True
            continue
        ...
        if re.match(r"^deallocate\s*\(\s*[a-z_]\w*(?:\s*,\s*[a-z_]\w*)*\s*\)\s*$", low):
            if not unit_stack:
                in_implicit_main = True
            continue
        if re.match(r"^open\s*\(.*\)\s*$", low):
            if not unit_stack:
                in_implicit_main = True
            continue
        if re.match(r"^close\s*\(.*\)\s*$", low):
            if not unit_stack:
                in_implicit_main = True
            continue
